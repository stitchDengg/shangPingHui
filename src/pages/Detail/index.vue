        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a href="javascript:" @click="addOrUpdateShopCat(skuid,buyValue)">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    // 更改产品选择高亮
    changeActive(saleAttrVake, arr) {
      arr.forEach((element) => {
        element.isChecked = "0";
      });
      saleAttrVake.isChecked = "1";
    },
    increase() {
      this.buyValue++;
    },
    decrease() {
      this.buyValue--;
      if (this.buyValue < 1) this.buyValue = 1;
    },
    // 当用户修改购物车数量表单时候触发
    changeBuyValue(e) {
      // 通过将输入的值乘以1将非数字字符排除掉
      ...
      if (isNaN(value) || value < 1) {
        ...
        // 如果是正常数组，需要有取整操作来避免是小数
        this.buyValue = parseInt(value);
      }
    },
    // 添加或修改购物车
    addOrUpdateShopCat(){
      // 1.跳转到购物车第一件事就是将参数发送给服务器
      let res = this.$store.dispatch('addOrUpdateShopCat',{skuid:this.skuid,skuNum:this.buyValue});
      //2. 请求成功以后就进行路由跳转
      res.then((res) => {
        console.log("加入购物车成功");
        // 3.进行路由跳转并且传递相应的信息
        window.sessionStorage.setItem('SKUIINFO',JSON.stringify(this.skuInfo));
        this.$router.push({
          name:'AddCartSuccess',
          query:{
            skuNum:this.buyValue,
          }
        })
      }).catch(e => {
        console.log(e);
